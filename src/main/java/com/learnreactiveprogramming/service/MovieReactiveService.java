...
public class MovieReactiveService {
    ...
    public Flux<Movie> getAllMovies_RestClient() {
        final Flux<MovieInfo> movieInfoFlux = movieInfoService.retrieveAllMovieInfo_RestClient();
        return movieInfoFlux.flatMap(movieInfo -> {
            Mono<List<Review>> reviewsMono = reviewService.retrieveReviews_RestClient(movieInfo.getMovieInfoId())
                    .collectList();
            return reviewsMono.map(reviews -> new Movie(movieInfo, reviews));
        }).onErrorMap((ex) -> {
            log.error("Exception ::: " + ex);
    ...
    public Mono<Movie> getMovie_RestClient(long movieId) {
        var movieInfoMono = movieInfoService.retrieveMovieInfo_RestClient(movieId);
        var reviewMono = reviewService.retrieveReviews_RestClient(movieId).collectList();
        return movieInfoMono.zipWith(reviewMono, (movieInfo, review) -> new Movie(movieInfo, review));
    }
